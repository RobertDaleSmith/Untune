...
class DatabaseManager {
    ...
    private init() {
        do {
            ...
            let dbURL = appSupport.appendingPathComponent("library.db")
    ...
    func syncMetadataBatch(tracks: [SyncClient.SyncTrackMeta], playlists: [SyncClient.SyncPlaylistMeta]) throws {
        try dbPool.writeWithoutTransaction { db in
            // Disable foreign keys entirely for sync — the desktop is the source of truth
            ...
            for syncTrack in tracks {
                ...
                if try TrackRecord.fetchOne(db, key: track.id) != nil {
                    try db.execute(
                        sql: """
                            UPDATE tracks SET
            ...
            // Remove playlists and tracks not in manifest (desktop is source of truth)
            ...
            if !manifestPlaylistIds.isEmpty {
                ...
                // Delete playlistTracks for removed playlists
                try db.execute(
    ...
    func upsertTrack(_ track: Track) throws {
        try dbPool.write { db in
            ...
            if try TrackRecord.fetchOne(db, key: track.id) != nil {
                // Update all metadata fields but preserve localFilePath and fileDownloaded
                try db.execute(
                    sql: """
                        UPDATE tracks SET
                            persistentId = ?, title = ?, artist = ?, albumArtist = ?,
    ...
    func upsertPlaylist(_ playlist: Playlist) throws {
        try dbPool.write { db in
            let record = PlaylistRecord.from(playlist)
            if try PlaylistRecord.fetchOne(db, key: playlist.id) != nil {
                try db.execute(
                    sql: """
                        UPDATE playlists SET
                            persistentId = ?, name = ?, isSmart = ?, isFolder = ?,
    ...
    func replacePlaylistTracks(_ playlistId: Int64, trackIds: [Int64]) throws {
        try dbPool.write { db in
    ...
    func updateTrackFilePath(trackId: Int64, localPath: String) throws {
        try dbPool.write { db in
            try db.execute(
                sql: "UPDATE tracks SET localFilePath = ?, fileDownloaded = 1 WHERE id = ?",
                arguments: [localPath, trackId]
            )
        }
    }
    ...
    func downloadedTrackCount() throws -> Int {
        try dbPool.read { db in
            try TrackRecord.filter(Column("fileDownloaded") == true).fetchCount(db)
        }
    }
}
