    ...
    <div ref={parentRef} className="flex-1 overflow-auto">
      <table className="w-full border-collapse text-xs">
        <thead className="sticky top-0 z-10 bg-neutral-900">
          <tr>
            <th className="px-2 py-1 text-left font-medium text-neutral-400 border-b border-neutral-800 w-[300px]">Artist</th>
            <th className="px-2 py-1 text-left font-medium text-neutral-400 border-b border-neutral-800 w-[80px]">Albums</th>
        ...
        <tbody>
          ...
          {virtualRows.map((vRow) => {
            ...
            return (
              ...
              >
                <td className="px-2 py-0 truncate text-neutral-200" style={{ maxWidth: 300 }}>{artist.name}</td>
                <td className="px-2 py-0 text-neutral-400">{artist.albumCount}</td>
                <td className="px-2 py-0 text-neutral-400">{artist.trackCount}</td>
              </tr>
            );
          })}
          {paddingBottom > 0 && (
